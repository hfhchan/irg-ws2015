...
<section class=ws2015_comments>
	<h2>IRGN2223 IRG Working Set 2015 Version 5.0 -- Identify Unprocessed Comments</h2>
	<p>
...
foreach ($chunks as $type => $chunk) {
	...
	foreach ($chunk as $cm) {
		...
		if ($cm->type == 'NORMALIZATION') {
			if (DBComments::find('MISDESIGNED_GLYPH', $cm->comment)) {
				continue;
			}			
		}
